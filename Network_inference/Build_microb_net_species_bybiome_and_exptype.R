...
constructAndSaveNetwork <- function(networkType, data, thresh, taxRank="Genus_Species", seed=123, 
...
option_list <- list(
  ...
  make_option(c("-b", "--biome_exptype"), type="character", default="Wastewater_metagenomic",
              help="Biome and experiment type", metavar="biome_exptype")
)
...
valid_biome_exptypes <- c("Wastewater_Activated_Sludge_assembly", "Wastewater_Activated_Sludge_metagenomic",
                          "Wastewater_Activated_Sludge_metatranscriptomic", "Wastewater_assembly",
                          "Wastewater_Industrial_wastewater_assembly", "Wastewater_Industrial_wastewater_metagenomic",
                          "Wastewater_metagenomic", "Wastewater_metatranscriptomic",
                          "Wastewater_Water_and_sludge_assembly", "Wastewater_Water_and_sludge_metagenomic")

if (!opt$network %in% valid_network_types) {
  stop("Network type must be 'cclasso' or 'spring'")
}

if (!opt$biome_exptype %in% valid_biome_exptypes) {
  stop("Biome must be one of the specified options")
}

# Construct the file path for saving the phyloseq object
file_input_path <- paste0("Data/Biomes_exptypes/", opt$biome_exptype, "_species_phyloseqfile.rds")

# Load data
phyloseq_species <- readRDS(file_input_path)

# Create file_path
file_output_path <- paste0("Output/Biomes_exptypes/", opt$biome_exptype)
...
constructAndSaveNetwork(opt$network, phyloseq_species, opt$threshold, base_folder=net_path, 
                        file_name_prefix=network_prefix, biome_name=opt$biome_exptype)